...
class Music {
    ...
    companion object {
        ...
        // Returns the key at the position selected
        fun getKey(position: Int): String {
            return ALL_MUSIC_KEYS.split('|')[position]
        }
    }

    // Will store the names of the notes
    lateinit var notes: Map<String, String>
        private set
    ...
    // Returns, from a chord name, the locale name of the chord
    fun getLocaleChordName(chordName: String): String {
        if(chordName != "" && notes.containsKey(""+chordName[0]))
            return notes["" + chordName[0]] + chordName.substring(startIndex = 1)
        return chordName
    }
}